      ...
      {res && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: 11 }} className="mono">
            {res.stats?.retrieved} chunks · {res.stats?.latency_ms}ms
          </div>
          <div>
            <button className="aca-btn-ghost small" onClick={() => setShowChunks((v) => !v)}>
              {showChunks ? '▾' : '▸'} {res.chunks?.length} retrieved chunks
            </button>
          </div>
          {showChunks && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {res.chunks?.map((c, i) => {
                const rel = c.score >= 0.7 ? 'success' : c.score >= 0.4 ? 'warning' : 'error'
                return (
                  <div key={i} style={{
                    ...
                    <div className="aca-row" style={{ gap: 8, marginBottom: 6 }}>
                      <span className={'aca-badge ' + rel}>{c.score.toFixed(2)}</span>
                      <span className="mono" style={{ color: 'var(--text-primary)', fontSize: 11 }}>{c.doc_id}</span>
                      <span className="mono" style={{ color: 'var(--text-muted)', fontSize: 11 }}>#{c.chunk_index}</span>
                    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button className="aca-btn-primary" onClick={upload} disabled={!fileName || !uploadId}>
              Upload
            </button>
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button className="aca-btn-primary" onClick={add} disabled={!docId || !text}>Add</button>
          </div>
        </div>
      </div>
      <div>
            ...
            <tbody>
              {docs.map((d) => (
                <tr key={d.doc_id}>
                  ...
                  <td><span className="aca-badge muted">{d.type}</span></td>
                  <td className="mono" style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{d.chunks}</td>
                  <td>
